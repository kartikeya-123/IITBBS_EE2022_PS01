...
import FootballEventForm from "../Modals/FootballModal";
...
const SportsLayout = () => {
  ...
  const initData = async () => {
    const cricketRef = collection(db, "Football");
  ...
  const submitToFirebase = async (data) => {
    const FootballRef = collection(db, "Football");
    await setDoc(doc(FootballRef), data);
  };
...
const SportEvent = (event) => {
  ...
  const handleClick = () => {
    navigate(`/sport/Football/${event?.event?.id}`);
  };
  return (
    <Grid item>
      <Card
